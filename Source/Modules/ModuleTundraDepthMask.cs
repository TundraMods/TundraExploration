...
{
	public class ModuleTundraDepthMask : PartModule
	// Module Copied from Drew Cassidy at: 
	// https://github.com/drewcassidy/KSP-DepthMask , Full credit goes to them.
	{
		// Module Name definition for error logging
		private const string MODULENAME = "ModuleTundraDepthMask";
		...
		{
			Debug.LogError($"[{MODULENAME}] : [{part.partInfo?.name ?? part.name} {this.GetType()}] {message}");
		}
	}
}